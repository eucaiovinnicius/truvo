...
export class EventContextQualityService {
  ...
  async evaluate(workspaceId: string, request: { requiredDimensions?: QualityDimension[]; outcomeNamespace?: string; outcomeKey?: string; historicalWindowDays?: number } = {}, database: Database = this.db) {
    const now = new Date();
    const [eligibleRow] = await database.execute(sql`select count(*)::int as count from customers where workspace_id=${workspaceId} and deleted_at is null and status <> 'merged'`);
    const [identityRow] = await database.execute(sql`select count(distinct c.id)::int as count from customers c join customer_identifiers i on i.workspace_id=c.workspace_id and i.customer_id=c.id where c.workspace_id=${workspaceId} and c.deleted_at is null and i.deleted_at is null`);
    ...
    for (const [dimension, table] of Object.entries(tables)) { const [r] = await database.execute(sql.raw(`select count(distinct customer_id)::int as count from ${table} where workspace_id = '${workspaceId.replace(/'/g, "''")}' and customer_id is not null`)); const observed = count(r); dimensions[dimension] = { eligible, observed, coverage: eligible ? observed / eligible : null }; }
    const [crmRow] = await database.execute(sql`select count(*)::int as count from crm_accounts where workspace_id=${workspaceId} and deleted_at is null`); dimensions.crm = { eligible, observed: count(crmRow), coverage: eligible ? count(crmRow) / eligible : null };
    const [outcomeRow] = await database.execute(sql`select count(*)::int as count from customer_outcomes where workspace_id=${workspaceId}`); const outcomes = count(outcomeRow);
    ...
    const connections = await database.execute(sql`select id, provider, lifecycle_state, credential_status, last_sync_at from connector_connections where workspace_id=${workspaceId} and role <> 'destination'`);
    ...
    for (const c of connections as unknown as Array<{ id: string; provider: string; lifecycle_state: string; credential_status: string; last_sync_at: Date | null }>) {
      const [checkpoint] = await database.execute(sql`select updated_at from connector_sync_checkpoints where workspace_id=${workspaceId} and connection_id=${c.id} order by updated_at desc limit 1`);
      const [recentRateLimit] = await database.execute(sql`select id from connector_sync_runs where workspace_id=${workspaceId} and connection_id=${c.id} and status='rate_limited' and created_at > now() - interval '2 hours' limit 1`);
      const checkpointAt = (checkpoint as { updated_at?: Date } | undefined)?.updated_at;
      const age = checkpointAt ? now.getTime() - new Date(checkpointAt).getTime() : c.last_sync_at ? now.getTime() - new Date(c.last_sync_at).getTime() : Infinity;
      ...
      else if (age > 48 * 60 * 60_000 && !recentRateLimit) issues.push({ stableKey: `source:stale:${c.id}`, category: 'freshness', severity: 'warning', connectionId: c.id, sourceNamespace: c.provider, actionCode: 'wait_for_backfill', details: { ageHours: Math.round(age / 3600000), basis: checkpointAt ? 'checkpoint' : 'connection_last_sync' } });
      freshness.push({ connectionId: c.id, provider: c.provider, state: recentRateLimit ? 'rate_limited_backoff' : c.lifecycle_state, ageHours: Number.isFinite(age) ? Math.round(age / 3600000) : null });
    }
    const [delivery] = await database.execute(sql`select coalesce(sum(greatest(attempt - 1, 0)), 0)::int as replay_attempts, coalesce(max(attempt), 1)::int as max_attempt from connector_sync_runs where workspace_id=${workspaceId} and kind='webhook_ingest'`);
    const replayAttempts = Number((delivery as { replay_attempts?: number })?.replay_attempts ?? 0); const duplicateSummary = { deliveryReplayAttempts: replayAttempts, canonicalNaturalKeyDefects: 0, canonicalUniquenessProtected: true, authority: 'connector_sync_runs.idempotency_key + canonical natural unique indexes' };
    if (replayAttempts >= 10) issues.push({ stableKey: 'duplicate:delivery_retry_pressure', category: 'duplicate', severity: 'warning', actionCode: 'investigate_duplicate_source', details: duplicateSummary });
    ...
    await this.persistIssues(workspaceId, issues, database);
    ...
    const historyDays = request.historicalWindowDays ?? 0; const reasonCodes = [...(critical ? ['blocking_quality_issues'] : []), ...((request.outcomeKey && outcomes === 0) ? ['insufficient_outcome_history'] : [])]; const readiness = { status: critical === 0 && eligible > 0 && (!request.outcomeKey || outcomes > 0) ? 'ready' : 'not_ready', eligibleCustomerCount: eligible, labeledOutcomeCount: outcomes, positiveOutcomeCount: null, historicalWindowDays: historyDays, identityCoverage: eligible ? identity / eligible : null, requiredDimensions: request.requiredDimensions ?? [], sourceFreshness: freshness, blockingIssues: critical, warnings, reasonCodes, actionCodes: issues.filter((i) => i.actionCode).map((i) => i.actionCode) };
    await database.execute(sql`insert into quality_evaluations (workspace_id, evaluated_at, data_health_score, data_health_status, context_coverage_score, identity_coverage, dimensions, source_freshness, duplicate_summary, critical_count, warnings_count, radar_readiness, updated_at) values (${workspaceId}, now(), ${dataHealthScore}, ${critical ? 'blocker' : warnings ? 'warning' : 'healthy'}, ${contextCoverageScore}, ${eligible ? Math.round(identity / eligible * 100) : 0}, ${JSON.stringify(dimensions)}::jsonb, ${JSON.stringify(freshness)}::jsonb, ${JSON.stringify(duplicateSummary)}::jsonb, ${critical}, ${warnings}, ${JSON.stringify(readiness)}::jsonb, now()) on conflict (workspace_id) do update set evaluated_at=excluded.evaluated_at, data_health_score=excluded.data_health_score, data_health_status=excluded.data_health_status, context_coverage_score=excluded.context_coverage_score, identity_coverage=excluded.identity_coverage, dimensions=excluded.dimensions, source_freshness=excluded.source_freshness, duplicate_summary=excluded.duplicate_summary, critical_count=excluded.critical_count, warnings_count=excluded.warnings_count, radar_readiness=excluded.radar_readiness, updated_at=now()`);
    return { dataHealth: { score: dataHealthScore, status: critical ? 'blocker' : warnings ? 'warning' : 'healthy', rules: { blockerPenalty: 30, warningPenalty: 10 } }, contextCoverage: { score: contextCoverageScore, dimensions }, identityCoverage: eligible ? identity / eligible : null, sourceFreshness: freshness, duplicateSummary, issues, criticalCount: critical, warningsCount: warnings, evaluatedAt: now.toISOString(), radarReadiness: readiness };
  }
  private async persistIssues(workspaceId: string, issues: QualityIssueInput[], database: Database = this.db) { const seen = new Set(issues.map((i) => i.stableKey)); for (const issue of issues) await database.execute(sql`insert into quality_issues (workspace_id,id,stable_key,category,severity,status,source_namespace,connection_id,stream_key,entity_type,entity_id,event_name,sample_context,action_code,details,first_seen_at,last_seen_at,occurrence_count,updated_at) values (${workspaceId},${randomUUID()},${issue.stableKey},${issue.category},${issue.severity},'active',${issue.sourceNamespace ?? null},${issue.connectionId ?? null},${issue.streamKey ?? null},${issue.entityType ?? null},${issue.entityId ?? null},${issue.eventName ?? null},${JSON.stringify(redactSample(issue.sampleContext))}::jsonb,${issue.actionCode ?? null},${JSON.stringify(issue.details ?? {})}::jsonb,now(),now(),1,now()) on conflict (workspace_id,stable_key) do update set severity=excluded.severity,status='active',last_seen_at=now(),occurrence_count=quality_issues.occurrence_count+1,resolved_at=null,updated_at=now()`); const active = await database.execute(sql`select stable_key from quality_issues where workspace_id=${workspaceId} and status='active'`); for (const row of active as unknown as Array<{ stable_key: string }>) if (!seen.has(row.stable_key)) await database.execute(sql`update quality_issues set status='resolved', resolved_at=now(), updated_at=now() where workspace_id=${workspaceId} and stable_key=${row.stable_key}`); }
