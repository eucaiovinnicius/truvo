...
export class EventContextQualityService {
  ...
  async evaluate(workspaceId: string, request: { requiredDimensions?: QualityDimension[]; outcomeNamespace?: string; outcomeKey?: string; historicalWindowDays?: number } = {}) {
    ...
    for (const c of connections as unknown as Array<{ id: string; provider: string; lifecycle_state: string; credential_status: string; last_sync_at: Date | null }>) {
      const [checkpoint] = await this.db.execute(sql`select updated_at from connector_sync_checkpoints where workspace_id=${workspaceId} and connection_id=${c.id} order by updated_at desc limit 1`);
      const [recentRateLimit] = await this.db.execute(sql`select id from connector_sync_runs where workspace_id=${workspaceId} and connection_id=${c.id} and status='rate_limited' and created_at > now() - interval '2 hours' limit 1`);
    ...
    const [delivery] = await this.db.execute(sql`select coalesce(sum(greatest(attempt - 1, 0)), 0)::int as replay_attempts, coalesce(max(attempt), 1)::int as max_attempt from connector_sync_runs where workspace_id=${workspaceId} and kind='webhook_ingest'`);
