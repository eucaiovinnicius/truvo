...
describe('Radar Postgres runtime closure', { concurrency: 1 }, () => {
  ...
  test('lifecycle success, concurrent train replay, cosmetic rename, pause and archive are durable', async () => {
    ...
    await radarService.reportTrainingResult(WORKSPACE_A, created.radar.id, request.definition_version, request.id, { status: 'succeeded', modelReference: 'model-test-radar-123-v1' });
    await radarService.reportTrainingResult(WORKSPACE_A, created.radar.id, request.definition_version, request.id, { status: 'succeeded', modelReference: 'model-test-radar-123-v1' });
    ...
    assert.deepEqual({ definitions: Number((history as { definitions: number }).definitions), requests: Number((history as { requests: number }).requests) }, { definitions: 1, requests: 1 });
  ...
  test('Postgres catalog contains Radar ownership/version/training uniqueness constraints', async () => {
    ...
    assert.ok(names.has('radar_training_requests_one_per_definition_uq'));
