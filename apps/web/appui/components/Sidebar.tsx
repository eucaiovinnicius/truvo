...
export default function Sidebar({ 
  ...
  const workspaces = ['Truvo Global Store', 'Alpha Electronics', 'BETA Cosmetics'];
        ...
        {showWorkspaceDropdown && (
          <div className="absolute top-full left-4 right-4 mt-1 bg-white border border-slate-100 rounded-lg shadow-lg z-50 py-1">
            {workspaces.map((ws) => (
              ...
              >
                {ws}
                {ws === workspaceName && <Check className="w-3.5 h-3.5 text-teal-600" />}
              </button>
            ))}
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="overflow-hidden">
              <span className="text-xs font-semibold text-slate-800 block truncate">{profile.fullName || 'Alex Mercer'}</span>
              <span className="text-[10px] font-medium text-slate-500 block truncate">{profile.email || 'alex@truvo.ai'}</span>
            </div>
          </div>
