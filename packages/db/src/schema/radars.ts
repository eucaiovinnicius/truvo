...
}, (t) => ({ pk: primaryKey({ columns: [t.workspaceId, t.id] }), idempotency: uniqueIndex('radar_training_requests_idempotency_uq').on(t.workspaceId, t.radarId, t.definitionVersion, t.idempotencyKey), oneRequestPerDefinition: uniqueIndex('radar_training_requests_one_per_definition_uq').on(t.workspaceId, t.radarId, t.definitionVersion) }));
